...
namespace GeodicBankAPI.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public UserController(IConfiguration config, IUser user, ILogger<UserController> logger)
        {
            _config = config;
            _user = user;
            _logger = logger;
        ...
        public ActionResult VerifyUser([FromBody] LoginRequest requestedStaff)
        {
            ...
            using (var dbContext = new FinancialDbContext())
            {
                if (requestedStaff.AuthKey == _config["Tokens:AuthKey"] || requestedStaff.AuthKey == _config["Tokens:AuthKey2"])
                {
                    ...
                    var possibleStaff = dbContext.Users.Where(s => s.FirstName.ToLower().Trim() == userName).FirstOrDefault();

                    if (possibleStaff != null)
                    {
                        isValid = BCrypt.Net.BCrypt.Verify(requestedStaff.Password, possibleStaff.Password.ToString());
                    }
                    if (possibleStaff != null && isValid)
                    {
                        ...
                        var claims = new[]
                        {
                            new Claim("Username", possibleStaff.FirstName),
                            new Claim(ClaimTypes.NameIdentifier, possibleStaff.Id.ToString()),
                            new Claim("UserId", possibleStaff.Id.ToString()),
                            new Claim("UserFullName", $"{possibleStaff.LastName} {possibleStaff.FirstName}")
                        };
                        ...
                        var data = new List<dynamic> {
                                new {
                                    ...
                                    //Role = role,
                                    StaffMember = possibleStaff!=null?possibleStaff:new User(),                                   
